#!/usr/bin/env python3
"""
Aggressive memory monitoring script for the music application.
This script monitors memory usage and restarts services immediately when memory gets high.
"""
...
class AggressiveMemoryMonitor:
    def __init__(self, memory_threshold=70, check_interval=5, restart_threshold=80):
        self.memory_threshold = memory_threshold
        self.restart_threshold = restart_threshold
        self.check_interval = check_interval
        self.restart_count = 0
        self.max_restarts = 10
        self.restart_cooldown = 60  # 1 minute
        self.last_restart = 0
    ...
    def force_garbage_collection(self):
        ...
        try:
            ...
            gc.set_threshold(10, 2, 2)  # Very aggressive GC settings
    ...
    def restart_service(self, service_name):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"✅ {service_name} restarted successfully (restart #{self.restart_count})")
                return True
            else:
            ...
            print(f"⏰ Restart timeout for {service_name} - service may be unresponsive")
            return False
        except Exception as e:
    ...
    def log_memory_status(self, mem_info):
        ...
        try:
            ...
            with open('aggressive_memory_monitor.log', 'a') as f:
    ...
    def monitor(self, duration_seconds=None):
        ...
        print("🔍 Aggressive Memory Monitor Starting...")
        print("=" * 60)
            ...
            while True:
                ...
                if system_memory_percent > self.restart_threshold:
                    ...
                    if mem_info_after and mem_info_after['system_memory_used_percent'] > self.restart_threshold:
                        print("🔄 Memory still high after cleanup, restarting services...")
                        
                        # Restart services in order of priority
                        if self.restart_service("app"):
                            print("✅ App service restarted")
                        if self.restart_service("fastapi"):
                            print("✅ FastAPI service restarted")
                        if self.restart_service("liquidsoap"):
                            print("✅ Liquidsoap service restarted")
                        
                        print("✅ Services restarted, waiting for stabilization...")
                    ...
                    if warning_count % 2 == 0:  # Every 2nd warning
                        self.force_garbage_collection()
                else:
...
def main():
    if len(sys.argv) > 1:
        try:
            ...
            print("Usage: python aggressive_memory_monitor.py [duration_seconds]")
            sys.exit(1)
    else:
        duration = None
    
    monitor = AggressiveMemoryMonitor(
        memory_threshold=70,  # Warning at 70%
        restart_threshold=80,  # Restart at 80%
