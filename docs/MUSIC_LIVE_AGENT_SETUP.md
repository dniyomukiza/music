...
1. **WebSocket URL**: `/ws/music/{user_id}/{session_id}?token=...` – pass `user_id` from session and include the signed Music Live token generated by the Flask template helper. Guest sockets may use `user_id=0` without a token.
